...
export interface EntryItemDisplayComponentProps {
    ...
    date: Date,
    emoji: string,
    onEditClick: (attribute: string) => void
}

export default function EntryItemDisplayComponent(props: EntryItemDisplayComponentProps) {
    const [displayDate, _] = useState<string>(isToday(props.date) ? "Today" : props.date.toLocaleDateString())
    return (
        <div className="flex flex-col rounded-md bg-ivory text-ivory-foreground">
            {/* some header */}
            <div className="flex flex-row justify-between text-baby-blue-foreground">
                <div className="
                    p-2
                    bg-baby-blue
                    text-baby-blue
                ...
                ><span className="text-baby-blue-foreground">{props.header}</span></div>
                
                <Button variant="icon" onClick={() => props.onEditClick(props.attribute)} size="icon" type="button" aria-label="Edit" className="mt-2 mr-2 bg-transparent hover:bg-ivory hover:text-coral-foreground">
            ...
            {/* content */}
            <div className="p-2 italic flex-grow">
                <p className="line-clamp-2">{props.content}</p>
            </div>

            {/* footer */}
            <div className="flex  items-center justify-between p-2 mr-2">
                <span className="text-xs uppercase">{displayDate}</span>
                <span className="text-white">{props.emoji}</span>
            </div>
        </div>
    )
}
